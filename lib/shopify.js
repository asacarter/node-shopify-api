...
ShopifyAPI.prototype.buildAuthURL = function(mode = 'offline') {
...
ShopifyAPI.prototype.exchange_temporary_token = function(params, callback) {
	var data = {
	...
	if (!self.is_valid_signature(params)) {
...
ShopifyAPI.prototype.makeRequest = function(endpoint, method, data, callback, retry) {
	if (!endpoint.includes('oauth')) {
		const regex = /\/admin/;
		endpoint = endpoint.replace(regex, `/admin/api/${this.config.shopify_version}`);
	}
		...
		response.on('end', function() {
			...
			if ((response.statusCode >= 200 || response.statusCode <= 299) && self.has_header(response, 'http_x_shopify_shop_api_call_limit')) {
				...
				if (self.config.backoff_level) {
					...
					limit_rate = (api_limit >= (self.config.backoff || 35));
				}
